      ...
      <div class="col-md-8 grid-margin stretch-card">
        <div class="card">
          <div class="card-body">
            ...
            <!-- <div class="form-group">
              ...
              <div class="dropLanguage" v-if="showLang.km == true">
                ...
                >
                  ...
                  <TinyMce
                    v-if="index != 0"
                    v-model="objData.km[index].content"
                  />
                </div>
              </div>
            </div> -->
            ...
            <div class="form-group">
              <label
                >Thêm biến thể (Bạn có thể thêm biến thể nếu sản phẩm này có
                nhiều lựa chọn như kích cỡ hoặc màu sắc. )</label
              >
              <vs-switch v-model="variantstatus">
            ...
            <div
              class="variant"
              v-if="variantstatus == true"
              style="background: #f6f6fb; padding: 7px"
            >
              <div class="form-group">
                <vs-select
                  autocomplete
              ...
              <div
                class="viarant"
                v-if="objData.status_variant == 1 && variant_value.length > 0"
              >
                <div
                  class=""
                  v-for="(item, index) in variant_value"
                  :key="index"
                >
                  ...
                  <ul class="centerx">
                    <li v-for="(itemval, key) in item.value" :key="key">
                      <vs-checkbox
                        :vs-value="itemval.name"
                        @change="getValuebuildSkua(itemval.name, item.name)"
                        >{{ itemval.name }}</vs-checkbox
                      >
                    </li>
                  </ul>
                </div>
                <div>
                  <vs-table :data="objData.lungtung">
                    ...
                    <template slot-scope="{ data }">
                      <vs-tr
                        :key="indextr"
                        v-for="(tr, indextr) in objData.lungtung"
                      >
                        <vs-td>
...
export default {
  ...
  methods: {
    ...
    getValuebuildSkua(variant_value, variant) {
      if (this.linhtinh.length == 0) {
        ...
        this.linhtinh.forEach((element, key) => {
          if (this.linhtinh.some((data) => data.display_name === variant)) {
            ...
            if (
              !element.option_values.some(
                (data) => data.label === variant_value
              ) &&
              this.linhtinh[key].display_name == variant
            ) {
              var obj = {};
              ...
              element.option_values.some((data) => data.label === variant_value)
            ) {
              const idxObj = this.linhtinh[key].option_values.findIndex(
                (obj) => {
      ...
      this.objData.lungtung = sets.map((set) => ({
        ...
        version: set.join("-"),
        sku: "",
        option_values: set.map((label) => ({
          option_id: id_obj[label].value_id,
          id: id_obj[label].id,
        })),
      }));
    },
    ...
    choiseVariant(event) {
      ...
      this.listVariantValue({ id: event.id })
        .then((response) => {
          if (this.variant_value.length == 0) {
            ...
            !this.variant_value.some((data) => data.name === event.name)
          ) {
            var obj = {};
            obj.name = event.name;
            obj.value = response.data;
    ...
    saveProducts() {
      ...
      if (this.objData.images.length == 0)
        this.errors.push("Vui lòng chọn ảnh");
      if (this.objData.category == 0)
  ...
  mounted() {
    ...
    this.listPartner()
      ...
      .catch((error) => {
        this.$error("Lỗi không thể lấy danh sách thương hiệu");
      });
  },
};
</script>
